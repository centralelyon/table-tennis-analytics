...
def calcul_diff_position(df):
    ...
    for i, row in df.iterrows():
        ...
        if time < time_mem :    # on change de match
            ...
            nombre_de_coups += 1
            if joueur == row['joueurA'] :
                coup_attaque_A += 1
            else :
                coup_attaque_B += 1
        elif type_coup == 'defensif' or type_coup == 'poussette' :
            nombre_de_coups += 1
            if joueur == row['joueurA'] :
                coup_attaque_B += 1
            else :
    ...
    return(diffsA, Fatig, Aggre, Angle)
    
## MORAL


def calcul_stress(df):  # différence entre stress et moral
    ...
    """
    calcul_scores(df)   # pour accèder au score
    Stress = []
    stressA = 1
    stressB = 1
        ...
        if faute == 'out' or faute == 'filet' :
            if joueur == row['joueurA'] :
                if scoreB != 11 :
                    ...
                    stressA += compteur_duree_echange
                faute_conseq_A += 1
                pt_gagne_conseq_A = 0
                ...
                if scoreA != 11 :
                    ...
                    stressB += compteur_duree_echange
                faute_conseq_B += 1
                pt_gagne_conseq_B = 0
            compteur_duree_echange = 0
